...
# 接続IP
define('CONNECT_IP', '192.168.0.12');

# ユーザ名
define('USER_NAME', 'root');

# パスワード
define('PASSWORD', 'A4$xfc%3');

# 使用DB
define('USE_DATABASE', 'win');

# 画像格納ディレクトリ（fullpath）
define('HOUSE_IMG', '/usr/local/apache2/htdocs/img/attaches');

# 年齢認証用ディレクトリ（fullpath）
define('HOUSE_AGE', '/usr/local/apache2/htdocs/img/age');

# smartphone用ディレクトリ
define('SMART_AGE_DIR','http://test-evol.com/promotions/smartauth/');

# attachesディレクトリ
define('ATTACHES_DIR','/usr/local/apache2/htdocs/attaches');

# 動画格納ディレクトリ（fullpath、認証前に置きます
define('HOUSE_MOV', '/usr/local/apache2/htdocs/movie/not_certify');

# 最高サイズ
define('MAX_SIZE', '5000000000');

# テスト用ディレクトリ
define('HOUSE_TEST', '/usr/local/apache2/htdocs/attaches/test_dir');
...
class DataBase {
    ...
    function InsertData($UserId, $UserSiteCode, $Attached, $category) {
        ...
        //mail("seraphic.blue@docomo.ne.jp","a",$InsDataSql);
    ...
    function InsertAgeData($UserId, $UserSiteCode, $Attached, $category) {
        ...
        $InsDataSql .= $this->UseTable['0']['5']." = ".$category.", ";
        $InsDataSql .= $this->UseTable['0']['7']." = ".$current_timestamp."";
        #mail("kizu@d-ef.co.jp","a",$InsDataSql);
        $InsDataRtn  = mysql_query($InsDataSql);
    }
}
