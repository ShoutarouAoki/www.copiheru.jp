...
$default_allowed_html			= array(
		...
		'a'					=> array(
			'href'			=> array('minlen' => 3, 'maxlen' => 100),
			'class'			=> array (),
			'href'			=> array (),
			'id'			=> array (),
			'rel'			=> array (),
		...
		'div'				=> array(
			'align' 		=> 1,
			'style'			=> array ('background-color' => array(),'padding' => array()),
			'class'			=> array (),
			'id'			=> array (),
			'dummy'			=> array('valueless' => 'y')
		),
...
$default_allowed_protocols		= array('http', 'https', 'ftp', 'news', 'nntp', 'telnet','gopher', 'mailto', 'color');


function kses($string, $allowed_html=NULL, $allowed_protocols=NULL)
###############################################################################
...
###############################################################################
{
	...
	if(empty($allowed_html)){
		...
		global	$default_allowed_html;
		$allowed_html	= $default_allowed_html;

	}

	if(empty($default_allowed_protocols)){

		global	$default_allowed_protocols;
...
/*
  return preg_replace_callback('%(<'.   # EITHER: <
                      ...
                      '(>|$)'. # > or end of string
                      '|>)%', # OR: just a >
					  function ($matches) { return kses_split2('\\1', $allowed_html, $allowed_protocols); },
                      $string);
  ...
  return preg_replace('%(<'.   # EITHER: <
                      ...
                      '(>|$)'. # > or end of string
                      '|>)%e', # OR: just a >
                      "kses_split2('\\1', \$allowed_html, ".'$allowed_protocols)',
                      $string);
*/
	...
	return preg_replace_callback(
				...
				function ($matches) { 
					if(empty($allowed_html)){
						global	$default_allowed_html;
						$allowed_html		= $default_allowed_html;
						$allowed_html		= kses_array_lc($allowed_html);
					}
					if(empty($default_allowed_protocols)){
						global	$default_allowed_protocols;
						$allowed_protocols	= $default_allowed_protocols;
...
###############################################################################
{
	...
	# Is there a closing XHTML slash at the end of the attributes?s
	$xhtml_slash = '';
	...
	#print("CHECK3 : ".$attr2."<br />\n");

	//#print("pre : ".$attr2."\n");

 	$attr2	= preg_replace_callback('/[<>]/', function ($matches) { return ''; }, $attr2);

	#print("RESULT : ".$attr2."<br /><br />\n\n");
    ...
    switch ($mode)
    {
      case 0: # attribute name, href for instance
        ...
        if (preg_match('/^([-a-zA-Z]+)/', $attr, $match))
        {
          $attrname = $match[1];
          $working = $mode = 1;
          $attr = preg_replace_callback('/^[-a-zA-Z]+/', function ($matches) { return ''; }, $attr);

			#print("CASE:0 → ".$attr."<br />");
      ...
      case 1: # equals sign or valueless ("selected")
        ...
        {
          $working = 1; $mode = 2;
          $attr = preg_replace_callback('/^\s*=\s*/', function ($matches) { return ''; }, $attr);
			#print("CASE:1-1 → ".$attr."<br />");
          break;
        }
        ...
        if (preg_match('/^\s+/', $attr)) # valueless
        {
          ...
          $attrarr[] = array
                        ('name'  => $attrname,
                         ...
                         'whole' => $attrname,
                         'vless' => 'y');
          $attr = preg_replace_callback('/^\s+/', function ($matches) { return ''; }, $attr);
			#print("CASE:1-2 → ".$attr."<br />");
      ...
      case 2: # attribute value, a URL after href= for instance
         ...
         # "value"
        {
          ...
          $attr = preg_replace_callback('/^"[^"]*"(\s+|$)/', function ($matches) { return ''; }, $attr);
			...
			#print("CASE:2-1 → NAME : ".$attrname." / VALUE : ".$thisval."<br />");
...
###############################################################################
{
  ...
  $string = preg_replace_callback('/\xad+/', function ($matches) { return ''; } , $string); # deals with Opera "feature"

#print("<font color=blue>BAD PROTOCOL1 → STR : ".$string."</font><br />");
  ...
  while ($string != $string2)
  {
	...
	#print("<font color=blue>BAD PROTOCOL2 → STR : ".$string."</font><br />");
...
###############################################################################
{
  $string = preg_replace_callback('/\0+/', function ($matches) { return ''; } , $string);
  $string = preg_replace_callback('/(\\\\0)+/', function ($matches) { return ''; } , $string);
...
# preg_replace_callback(//e) seems to require this.
###############################################################################
{
...
###############################################################################
{
  return preg_replace_callback('%&\s*\{[^}]*(\}\s*;?|$)%', function ($matches) { return ''; } , $string);
...
#print("<font color=green>BAD PROTOCOL ONCE1 → STR : ".$string."</font><br />");
...
#print("<font color=orange>BAD PROTOCOL ONCE2 → STR : ".$string."</font><br />");

  $string2 = kses_decode_entities($string);

#print("<font color=orange>BAD PROTOCOL ONCE2 → STR2 : ".$string2."</font><br />");

  $string2 = preg_replace_callback('/\s/', function ($matches) { return ''; }, $string2);
...
#print("<font color=orange>BAD PROTOCOL ONCE2 → STR2 : ".$string2."</font><br />");
  ...
  $string2 = preg_replace_callback('/\xad+/', function ($matches) { return ''; }, $string2);
...
# Change back the allowed entities in our entity whitelist
  ...
  $string = preg_replace_callback('/&amp;#0*([0-9]{1,5});/',
                         function ($matches) { return kses_normalize_entities2($matches[0]); } ,
						 $string);
...
###############################################################################
{
  ...
  //$string = preg_replace_callback('/&#([0-9]+);/e', array(get_class($this), 'chr("\\1")'), $string);
  //$string = preg_replace_callback('/&#[Xx]([0-9A-Fa-f]+);/e', array(get_class($this), 'chr(hexdec("\\1"))'),$string);

  $string = preg_replace_callback('/&#([0-9]+);/', function ($matches) { return chr($matches[0]); },  $string);
  $string = preg_replace_callback('/&#[Xx]([0-9A-Fa-f]+);/', function ($matches) { return chr(hexdec($matches[0])); } , $string);
