...
class ImageModel{
	...
	**************************************************/
	...
	public function getImageList($post_data,$column=NULL){
		...
		if(!isset($post_data['payment'])){
			...
			$where					.= "AND (payment = :payment0 OR payment = :payment1) ";
			$array[':payment0']		 = 0;
			$array[':payment1']		 = $post_data['payment'];
		}
		
		//20181019 add by A.cos
		if(!isset($post_data['buy'])){
			$where					.= "AND buy = :buy ";
			$array[':buy']		 = 0;
		}else{
			$where					.= "AND (buy = :buy0 OR buy = :buy1) ";
			$array[':buy0']		 = 0;
			$array[':buy1']		 = $post_data['buy'];
		}
	...
	**************************************************/
	...
	public function getImageData($post_data,$column=NULL){
		...
		/* 
		///payment&buy行がDB上から消えていたため。20240207 delete by s-aoki
		...
		///
		*/
		if(isset($post_data['limit'])){//複数枚の画像を取り出す場合。20180125 update by A.cos
