...
/**************************************************
	...
	**************************************************/
	...
	public function getMailListPast($year, $exclusion_id, $post_data,$column=NULL){
		global $array_exit;
		# ERROR
		if(empty($post_data['user_id']) || empty($post_data['type']) || empty($year)){
			return FALSE;
		}
		...
		if($post_data['type'] == 1){
			...
			# 除外ID
			if(strlen($exclusion)){
				$where						.= "AND send_id NOT IN (".$exclusion.") ";
			}
		...
		# SEND ONLY
			...
			# 除外ID
			if(strlen($exclusion)){
				$where						.= "AND recv_id NOT IN (".$exclusion.") ";
			}
		# RECEIVE & SEND ALL
		}elseif($post_data['type'] == 3){
		...
		$rtn					 = $database->selectDb("mails_past_".$year,$column,$where,$array,$order,$limit,$group,1);
		$error					 = $database->errorDb("GET MAIL LIST PAST-".$year,$rtn->errorCode(),__FILE__,__LINE__);
		if(!empty($error)){ $output->outputError($error); }
