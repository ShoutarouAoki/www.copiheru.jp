...
class ItemuseModel{
	...
	**************************************************/
	...
	public function getItemuseData($post_data,$column=NULL){
		...
		$rtn						 = $database->selectDb($this->table,$column,$where,$array,$order,$limit,$group);
		$error						 = $database->errorDb("getItemuseData",$rtn->errorCode(),__FILE__,__LINE__);
		if(!empty($error)){ $output->outputError($error); }
	...
	**************************************************/
	...
	public function checkItemUseLimit($post_data,$column=NULL){
		...
		# COLUMN
		if(empty($column)){
			$column					 = "u.id as itemuse_id, u.itembox_id, u.limit_time, u.limit_count, u.status as itemuse_status, ";
			$column					.= "i.id as items_id, i.name, i.image, i.word, i.category, i.type, i.effect";
		}
		...
		while($data = $database->fetchAssoc($rtn)){
			...
			************************************************/
			...
			if($data['limit_time'] == 0 && $data['limit_count'] == 0){
				continue;
			}
			...
			***************************************/
			...
			if(!empty($end)){
				...
				# 処理したものを配列に入れて返す
				$list['items_id'][$i]				= $data['itembox_id'];
				$list['itemuse_id'][$i]				= $data['itemuse_id'];

				# itembox の IDを取得

				# JSON用にカンマ区切りも作成
				$list['end_list']					.= $data['itembox_id'].",";
		...
		if(!empty($update)){
			...
			************************************************/
			...
			if(!empty($end)){
				...
				# 使い切ったアイテムデータ渡す
				$calculation_data['end']			= $itemuse_data['itembox_id'];
