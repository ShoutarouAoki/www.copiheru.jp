...
$value_array				= ['page','id', 'event_id', 'set'];
...
//JavaScriptから非同期でPOSTでイベントIDが送られてきた場合
if(!empty($_POST['event_id'])){
	$eventmail_id						= $_POST['event_id'];
}else{
	if(!empty($data['event_id']) && strlen($data['event_id'])){
		$eventmail_id = $data['event_id'];
	}else{
		# イベントIDがないとリダイレクト
	...
	if($data['page'] == "detail" || $data['page'] == "more"){
		...
		************************************************/
		...
		if(!empty($character_data['id'])){
			...
			if(empty($children_id)){
				...
				if(!empty($children_check['id'])){
					...
					************************************************/
					...
					$mails_r_count						= $eventmailModel->getEventMailCount($eventmail_id,$mails_r_conditions);
					...
					if($mails_r_count > 0){
						...
						# 【UPDATE】event_mails
						$database->updateDb("event_mails",$mails_r_update,$mails_r_update_where,$mails_r_update_conditions);
					...
					$mails_s_count						= $eventmailModel->getEventMailCount($eventmail_id,$mails_s_conditions);
					...
					if($mails_s_count > 0){
						...
						# 【UPDATE】event_mails
						$database->updateDb("event_mails",$mails_s_update,$mails_s_update_where,$mails_s_update_conditions);
			...
			**	============================================
			**	eventmail_id				: イベントID
			**	$character_id			: 相手しているキャラの親ID
			**
			************************************************/
			# 
			$eventmail_sql  = "SELECT * FROM event_mail_settings";
			$eventmail_sql .= " WHERE site_cd = ".$members_data['site_cd'];
			$eventmail_sql .= " and event_id=".$eventmail_id." and character_id=".$parent_id;
			$eventmail_sql .= " order by id LIMIT 1";
			//echo $eventmail_sql;
			$eventmail_rtn = $database->query($eventmail_sql);
			//$database->errorDb("",$database->errno,__FILE__,__LINE__);
			//$database->errorDb("",$event_mail_character_list->errorCode(),__FILE__,__LINE__);

			$eventmail_data = $database->fetchAssoc($eventmail_rtn);
			//	$event_mail_settings_character_id[] = $event_mail_character_data['character_id'];
			//echo "<pre>";
			//print($eventmail_data);
			//echo "</pre>";
			...
			**	mailsの抽出ID振り分け
			**	============================================
			**	$character_id			: 画面開いた時に表示するevent_mailsデータのキャラID
			**	$post_character_id		: WEB画面で自動で読み込みにいくevent_mailsデータのキャラID
			**	メールしたことないユーザーは
			**	$character_idが親キャラID
			...
			************************************************/
			...
			$mails_rtn							= $eventmailModel->getEventMailList($eventmail_id,$mails_conditions,"*");
			...
			if($character_send_count == 0){
				...
				$last_mail_data						= $eventmailModel->getEventMailData($eventmail_id,$last_mail_conditions,"id");
			...
			************************************************/
			...
			if($data['page'] == "detail"){
				...
				************************************************/
				...
				if($character_data['word'] == 0){
					$media = explode(",", $eventmail_data["media"]);
				
					$i=0;
					foreach($media as $eventmail_display_image){

						$display_image['id'][$i]		= $i;//$attaches_data['id'];<-ビュー側で枚数のカウントをするためのものなので、暫定的に数値だけ入れておく
						$display_image['image'][$i]		= $eventmail_display_image;
						$i++;
					}
					# 画像枚数
					$attaches_count = $i;
			...
			************************************************/
			...
			if($check_mail_count >= $list){
				...
				# メールのやり取り総数
				$mail_count								= $eventmailModel->getEventMailCount($eventmail_id,$count_conditions);
			...
			************************************************/
			...
			if($character_data['word'] == 0){
				...
				************************************************/
				# 
				$eventmail_sql  = "SELECT * FROM event_mail_settings";
				$eventmail_sql .= " WHERE site_cd = ".$character_data['site_cd'];
				$eventmail_sql .= " and event_id=".$_POST['event_id']." and character_id=".$parent_id;
				$eventmail_sql .= " order by id LIMIT 1";
				$eventmail_rtn = $database->query($eventmail_sql);
				...
				foreach($media as $eventmail_display_image){
					...
					if($i == 0){
						$animation_image				= "<img src=\"".HTTP_ATTACHES."/".$eventmail_display_image."\" />";
					}

					$display_image						.= "<img src=\"".HTTP_ATTACHES."/".$eventmail_display_image."\" />";
	...
	# READ
		...
		if(!empty($mails_id) && is_numeric($mails_id)){
			...
			************************************************/
			...
			$mails_data							= $eventmailModel->getEventMailDataById($eventmail_id,$mails_conditions,$mails_column);
	...
	# READ
		...
		if(!empty($mails_id) && is_numeric($mails_id)){
			...
			************************************************/
			...
			# メール専用以外のイベント ID 代入 (ランキングイベントがあれば)
			...
			************************************************/
			...
			if(empty($error)){
				...
				# EVENTS_MAILS DATA 取得
				$eventmails_conditions					= [
					'id'							=> $mails_id
				];
				
				$mails_data							= $eventmailModel->getEventMailDataById($eventmail_id,$eventmails_conditions);
				
				# EVENTS_MAILS DATA レコードOK
				if(!empty($mails_data['id'])){
						...
						************************************************/
						...
						if(empty($point_result['error'])){
							...
							************************************************/
							...
							# 【UPDATE】 / event_mails
							$database->updateDb("event_mails",$mails_update,$mails_update_where,$mails_update_conditions);
							...
							************************************************/
							if(!empty($first_mail)){
								...
								$receive_count						= 0;
								$receive_count						= $eventmailModel->getEventMailCount($eventmail_id,$receive_conditions);
		...
		if(empty($error)){
			...
			************************************************/
			$mails_insert							= [
				...
				'naruto'							=> $character_data['naruto'],
				'event_id'							=> $eventmail_id
			];

			# 【INSERT】event_mails
			$insert_id								= $database->insertDb("event_mails",$mails_insert);


			/************************************************
			**
			**	event_mails insert ERROR
			**
			************************************************/
						...
						************************************************/
						...
						if(!empty($character_data['naruto'])){
							...
							************************************************/
							...
							$mails_count						= $eventmailModel->getEventMailCount($eventmail_id,$mails_conditions);
