...
class Database{
	...
	**************************************************/
	...
	public function connectDb($grant=NULL) {
		...
		if($grant === MASTER_ACCESS_KEY){
			...
			$this->debug_query .= "【DATABASE SELECT】&nbsp;→&nbsp;<span style=\"color: #00CCFF;\">MASTER</span>";
			$this->debug_query .= "\n<hr class=\"query_line\" />\n";
			$this->authority	= 1;
		}else{
			$db_ip				= DATABASE_IP_S;
			...
			$this->debug_query .= "【DATABASE SELECT】&nbsp;→&nbsp;<span style=\"color: #FF0000;\">SLAVE</span>";
			$this->debug_query .= "\n<hr class=\"query_line\" />\n";
			$this->authority	= NULL;
		}
	...
	**	queryExe
	**	----------------------------------------------
	**	DATABASE MYSQL QUERY -> FETCH
	**
	**************************************************/

	public function queryExe($sql=NULL){
 		if($sql){
			$result = $this->query($sql);
			if (!$result){
				return FALSE;
	...
	**************************************************/
	...
	public function arrangeData($table,$data,$purpose,$link=NULL){
		...
		$error		 = $this->errorDb("SHOW FIELDS : ARRANGE DATA",mysqli_errno(),__FILE__,__LINE__);
